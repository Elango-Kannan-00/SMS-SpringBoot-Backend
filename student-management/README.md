...
- `spring.jpa.hibernate.ddl-auto=update` will automatically create or update the `student` table based on the entity definition.
- The API currently returns a reduced response DTO for `POST`, `GET`, and `PUT` operations.
- There is no global exception handler in the current codebase, so unhandled exceptions may return the default Spring Boot error response.
...
curl -X POST http://localhost:8080/student \
  ...
  -d "{\"name\":\"John Doe\",\"email\":\"john@example.com\",\"department\":\"CSE\",\"mobileNo\":9876543210,\"age\":21,\"cgpa\":8.5}"
```
...
curl -X PUT http://localhost:8080/student/1 \
  -H "Content-Type: application/json" \
  -d "{\"name\":\"John Smith\",\"email\":\"johnsmith@example.com\",\"department\":\"ECE\",\"mobileNo\":9876543211,\"age\":22,\"cgpa\":8.9}"
```
...
This project is a Spring Boot-based student management API.
